...
8. Top-level Caddy roots (`/earth`, `/institution`, `/human`, `/llms.txt`, `/.well-known/agent.json`) are **not** always `/var/www/html/arif`. Copy those explicitly via `scripts/sync-serving-roots.sh`. Scar: WEB-001, split-roots 2026-08-25. `/human` without a static `index.html` in **both** `/var/www/html/human` and `/var/www/html/arif/human` falls through `@spa_routes` to the homepage shell.
